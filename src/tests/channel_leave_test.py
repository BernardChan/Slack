...
def test_leave_slackr_owner():
    ...
    channel_invite(ch.slackr_owner_token, ch.channel_id, ch.slackr_owner_id)
    assert_user_leave(ch.slackr_owner_token, ch.slackr_owner_id, True)

    workspace_reset()
    ch.init_helper()
    channel_invite(ch.slackr_owner_token, ch.private_channel_id, ch.slackr_owner_id)
...
def test_leave_member():
    ...
    channel_invite(ch.member_token, ch.channel_id, ch.member_id)
    channel_invite(ch.member_token, ch.private_channel_id, ch.member_id)
...
def test_leave_input_error():
    ...
    with pytest.raises(InputError):
        leave(ch.chan_owner_token, -100000)
    workspace_reset()
