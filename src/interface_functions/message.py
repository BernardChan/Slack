...
def message_edit(token, message_id, message):
    ...
    mess = DATABASE['messages'][message_id]['message']
    channel_id = DATABASE['messages'][message_id]['channel_id']
        
    # check if message is valid
    help.is_message_valid(token, mess, channel_id)
    
    #check if user is a member
    is_user_valid_channel_member(token, channel_id)
    
    
    #removing message
    if message == '':
        remove.remove_message(token, message_id)
    else:
